...
function s3Credentials(config, params) {
  return {
    ...
    //endpoint_url: "http://atlasresults.serversdev.getgo.com/atlas-loadtest/index.html",
...
function dateString() {
  var date = new Date().toISOString();
   var x=date.substr(0, 4) + date.substr(5, 2) + date.substr(8, 2);
   console.log(x);
   return x
}
...
function s3UploadPolicy(config, params, credential) {
  keyy = params.filename
  return {
    // expires one year into the future
    expiration: new Date((new Date).getTime() + (8760 * 60 * 60 * 1000)).toISOString(),
    conditions: [
